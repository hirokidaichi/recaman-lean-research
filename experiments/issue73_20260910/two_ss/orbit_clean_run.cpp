// SS=0 (clean) same-run min P2 pairs.
#include <array>
#include <cstdint>
#include <iostream>
#include <stdexcept>
...
int main(){
 ...
 I run_last_t=-1,run_last_d=-1,run_last_lead=-1; N nviol=0, gap2=0;
 std::cout<<"protocol=H-20260910-36 clean same-run min P2 pairs\n"<<std::flush;
     ...
     if(run_last_t>=0){
      ...
      if(nviol<=12)
       std::cout<<"VIOL={\"prev_t\":"<<run_last_t<<",\"prev_d\":"<<run_last_d
         <<",\"prev_lead\":"<<run_last_lead
         <<",\"t\":"<<t<<",\"d\":"<<d<<",\"lead\":"<<lead
         <<",\"gap\":"<<gap<<"}\n"<<std::flush;
     }
     run_last_t=t; run_last_d=d; run_last_lead=lead;
    }
   }
  }
  if(isS) signS[t]=true;
 ...
 std::cout<<"CLEAN_VIOL={\"n\":"<<nviol<<",\"gap2\":"<<gap2<<"}\n";
}
