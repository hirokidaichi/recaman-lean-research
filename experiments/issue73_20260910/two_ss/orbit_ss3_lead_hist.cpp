// SS=3 min P2 leading-run length histogram (a≥3 companions).
#include <array>
#include <cstdint>
#include <iostream>
#include <stdexcept>
...
int main(){
 ...
 std::array<N,16> leadc{}; N lead_ge16=0, ss3=0;
 std::cout<<"protocol=H-20260910-38 SS=3 leading-run histogram\n"<<std::flush;
 ...
 for(I n=1;n<=limit;++n){
  ...
  if(!isS){
   ...
   if(it!=latest.end()){
    ...
    if(ss==3){
     ++ss3;
     if(lead>=16) ++lead_ge16; else ++leadc[lead];
    }
  }
  }
  if(isS) signS[t]=true;
 ...
 if(ss3!=21727) throw std::runtime_error("ss3");
 std::cout<<"LEAD={";
 for(I i=0;i<16;++i){ if(i) std::cout<<","; std::cout<<"\"a"<<i<<"\":"<<leadc[i]; }
 std::cout<<",\"a_ge16\":"<<lead_ge16<<"}\n";
}
