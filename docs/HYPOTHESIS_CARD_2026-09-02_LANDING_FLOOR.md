...
## Exact mechanism（2026-09-02 夜、arc trace で確定）

`a(n) = k·n + r` と書くと両ステップとも `r → r − k` で、弧の底は弧の最後の遅延着地である。
comb の末端 `(c, v)`（`v−1` 既訪問）では +3 の候補 `c+v = a(c−1)` が必ず塞がれ `k=3` へ跳ねる
（`late_landing_popup`）。+4 の候補 `2c+v+2` が既訪問なら `k=3/4` に固定され、`k=3→2` の候補は
弧自身の `k=2` 上側 run の値なので全て塞がれ、剰余が尽きるまで遅延着地は起きない（`PopupLock`）。
深い弧 6 本の底はすべてこの事象で、`2c+v+2` は同じ弧が少し前に `k=2` 値として訪問していた。
従って **弧の底 = その弧で最初に `Φ = 2·時刻+高さ` が走行最大値から 3 以上下がった comb 末端の高さ**。
landing floor は「`Φ` の落差が 3 に達する高さは 852656 を下回らない」と言い換えられる。
...
3. `Φ` の走行最大値からの落差が初めて 3 に達する高さの法則を紙上で証明する。落差は comb 末端で
   `c+v−3` が既訪問のとき（`k=2/3` 段）に生じるので、帯 `(c, c+v)` の既訪問値の分布が鍵である。
   これが本丸。
