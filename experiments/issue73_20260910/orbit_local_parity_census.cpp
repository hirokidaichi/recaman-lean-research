// H-20260910-07b, derived from the frozen H-07 census: exact finite-history P2 supply census, with no lag cutoff.
#include <algorithm>
#include <array>
#include <cstdint>
#include <iostream>
...
struct Stats {
  ...
  std::map<N,N> clean_runs, clean_lag_bins;
...
int main() {
  ...
  for (I n=1;n<=limit;++n) {
    ...
    if(isS){lastS[t%2]=t;signS[t]=true;}
  ...
  std::cout << "PASS: exact local-parity coverage and hybrid S injection through 10000000 steps\n";
}
