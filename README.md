...
供給数の容量は、全 lag≤11 と SS を含まない clean な長窓の和集合まで、全周期で Lean 証明した。
標準 10^7 ステップでは有限 P2 供給加算の約 70.07% がこの範囲に入る。
残る SS を跨ぐ窓について、一つの SS に任意個の需要が集まる族を Lean で証明し、定数予算案を停止した。
SS1 窓の二族分類と SS1 単独の全周期容量は紙上で証明し、まれな型 B も標準 step 96,911,838 で独立再計算により確認した。
...
- Lean 4.33.1で固定
- Lean標準ライブラリのみを使用
- Leanソース302ファイル（79,957行、root・audit・認証済み深部traceを含む）
- 主要定理の公理監査を同梱
