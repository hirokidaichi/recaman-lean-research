// Isolated a=0 singleton SS=2: gap between the two SS edges.
#include <array>
#include <cstdint>
#include <iostream>
#include <stdexcept>
...
int main(){
 ...
 N sing=0;
 std::array<N,8> gapc{}; N gap_ge8=0;
 I pend_t=-1, pend_g=-1, pend_d=-1;
 std::cout<<"protocol=H-20260910-38 isolated singleton SS-gap\n"<<std::flush;
 ...
 for(I n=1;n<=limit;++n){
  ...
  if(pend_t>=0 && t==pend_t+1){
   if(isS){
    ++sing;
    if(pend_g>=8) ++gap_ge8; else if(pend_g>=0) ++gapc[pend_g];
    if(pend_g==5){
     std::string w;
     for(I j=1;j<=std::min<I>(pend_d,24);++j) w.push_back(signS[pend_t-j]?'S':'A');
     std::cout<<"GAP5={\"t\":"<<pend_t<<",\"d\":"<<pend_d<<",\"head\":\""<<w<<"\"}\n"<<std::flush;
    }
   }
   pend_t=-1;
  }
  if(!isS){
   ...
   if(it!=latest.end()){
    ...
    for(I j=1;j<d;++j){
     ...
     // newest-first SS at index j-1 if w[j-1]=w[j]=S i.e. signS[t-j] and signS[t-(j+1)]
     if(s && nxt){
      ++ss;
      if(first<0) first=j-1; else second=j-1;
     }
    }
    if(ss==2 && lead==0){
     pend_t=t; pend_d=d; pend_g=(first>=0 && second>=0)? (second-first) : -1;
    }
  }
  }
  if(isS) signS[t]=true;
 ...
 std::cout<<"GAPS={\"singleton\":"<<sing;
 for(I g=0;g<8;++g) std::cout<<",\"g"<<g<<"\":"<<gapc[g];
 std::cout<<",\"g_ge8\":"<<gap_ge8<<"}\n";
}
