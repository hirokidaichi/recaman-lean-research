...
さらに等号語では課金が一意に強制され、強制辺 1,110 本はすべて E-069 の oldest-S 先と一致した（E-178）。
一般には反証されている oldest-S が、等号が立つところでは唯一可能な課金だったことになる。
これで名前付き charge が全滅してきた理由が確定し、selector 探索は終了した。
その線形強化 `slack ≥ 衝突数` は周期18の `AAAASSAAAASAAASSSS` で反証した（E-180）。
代わりの局所目標は「高SS窓は自分の内部から削除可能な S を供出する」（E-179）で、これが次の gate である。
網羅は周期31まで拡張し、`|U|≤|D|` と `U≠A` の違反は0（E-175）。
Lean では、この分枝の全探索が黙って依存していた後退走査の停止則を定理にした（E-181、`PeriodicSupplyBound`）：
正の周期質量だけから `d≤p(p+1)` の有限検査に落ち、「どの lag でも供給されない」が kernel 検査可能になった。

2026-09-10の[3時間研究](docs/THREE_HOUR_RESEARCH_2026-09-10.md)で、任意の有限初期履歴と開始時刻からの
実更新が周期的になるなら、各加算位相に有限 P2 供給が必要になることを Lean で証明した（E-065）。
非正ドリフトの排除と初期履歴の処理を含み、周期 p の必要 lag は `0<d<p(p+1)` に収まる。

続く [共通端点の研究](docs/LOW_SS_ENDPOINT_CAPACITY_2026-09-10.md) で、SS（減算2連）が高々1個の
P2 窓を持つ加算位相の共同容量を、全周期・全 lag で Lean 証明した（E-128）。
clean と SS1 を一緒に扱い、NoSAAS・最小 lag を仮定しない。標準 10^7 ステップでは
...
- Lean 4.33.1で固定
- Lean標準ライブラリのみを使用
- Leanソース306ファイル（80,747行、root・audit・認証済み深部traceを含む）
- 主要定理の公理監査を同梱
...
- preloadなしの単一初期値generalized orbit 20,001本（内部供給burst use 1.27M件）には
  same-candidate linkが、strict-high形でもcorridor-faithfulなc-floor形（中間candidate ≥ c）でも0件。
  既知の多use反例の機構はpreloaded blockerなしでは一度も現れず、
  「generalized orbitにc-floor linkは存在しない」を`CONJECTURED`として登録
