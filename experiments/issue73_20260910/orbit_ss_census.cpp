// H-20260910-11, derived from the frozen H-10 census: exact finite-history P2 supply census, with no lag cutoff.
#include <algorithm>
#include <array>
#include <cstdint>
#include <iostream>
...
struct Stats {
  ...
  std::map<N,N> ss_hist, residual_ss_hist;
...
void Report(const Stats& s,N lo,N hi) {
  ...
  std::cout << ",\"adjacent_SS_count_hist\":"; PrintMap(s.ss_hist);
  std::cout << ",\"residual_adjacent_SS_count_hist\":"; PrintMap(s.residual_ss_hist);
  std::cout << "}\n" << std::flush;
}
...
int main() {
  ...
  for (I n=1;n<=limit;++n) {
    ...
    if (isS) {++st.S;} else {
      ...
      if (it==latest.end()) ++st.no_supply;
        ...
        I ss=ssTimes.end()-std::lower_bound(ssTimes.begin(),ssTimes.end(),t-d+1);
        I ssbin=ss<=10?ss:ss<=100?100:ss<=1000?1000:10000000;
        ++st.ss_hist[ssbin];
        if(!short11 && !clean)++st.residual_ss_hist[ssbin];
        if((ss==0)!=clean)throw std::runtime_error("no-SS clean characterization failure");
    ...
    if(isS){
      if(t>=1 && signS[t-1])ssTimes.push_back(t);
      if(t>=3 && signS[t-3] && !signS[t-2] && !signS[t-1])throw std::runtime_error("forbidden SAAS on canonical orbit");
      lastS[t%2]=t;signS[t]=true;subTimes[t%2].push_back(t);
    }
  ...
  std::cout << "PASS: SS-free iff local-clean for every supplied A; canonical SAAS absent; exact SS census through 10000000 steps\n";
}
