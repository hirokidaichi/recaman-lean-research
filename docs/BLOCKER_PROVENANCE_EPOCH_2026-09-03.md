...
|---|---|---|---|---|
| `entry23` | 6,839 / 12,465（100% of 同じ弧） | 1,100 / 1,501（`n ≈ c/2`） | 96 / 260 | gap `c−n ∈ [4,8)` が 732 / 1,491（ちょうど 7 が計 2,222）：comb 自身の歯 4 の加算値 `c+v−3`（`CombExit`） |
...
- 同じ弧の blocker の初訪問は弧の後半に集中する：`n/c ≥ 0.912`（test）、`≥ 0.992`（l3）、`≥ 0.942`（lockcand）、
  `≥ 0.704`（entry23）、`≥ 0.621`（bandexit）。
- 前の弧の寄与は level が距離とともに上がる：test は前の弧の q=4（`n/c ∈ [0.444, 0.618]`）と前々の弧の q≥6
  （`[0.342, 0.424]`）、l3 は q=5（`[0.619, 0.715]`）と q=6（`[0.480, 0.496]`）、entry23 は前の弧の q=2 と
  前々の弧の q=4（`[0.236, 0.465]`）、bandexit は q=2・q=3（`[0.323, 0.338]`）・q=4（`[0.206, 0.465]`）。
- bandexit の同じ弧の 5,021 件のうち 2,031 件は gap 2（帯値 `a(n)−1` は chain 自身の直前の下側値で、遅延着地に
  中断された chain へ comb の加算が 1 つ上から再入する）、405 件は gap 6。
- lock の結末と T は test の描像を変えない：どの cell でも同じ弧は全て q=2、前の弧は全て q≥3。
- ステップ種は run の側と一致する（S ≈ lower、A ≈ upper）。
