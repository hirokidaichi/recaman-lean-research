...
### low-SS endpoint capacity — 整理用

供給窓ごとの `ssCount≤1` を low-SS と呼ぶ。周期全体の SS 個数の制限ではない。
任意の low-SS P2 窓には S で終わる P2 prefix があり、現在 A の異なる供給源は
同じ最古端点を共有できない。この一つの写像で clean と SS1 の共同容量を全周期で
Lean 証明した（E-128）。NoSAAS・最小 lag・lag 上限は不要。
旧 one-SS run 写像と clean 写像の衝突は、この新写像で解消した。
SS2 への同じ prefix 正規化は標準step115で偽（E-130）。旧shortとの和集合もまだ未証明。
