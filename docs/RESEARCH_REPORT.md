...
`PermanentAboveCorridorWindow`はpost-legal terminalのall-forced suffixを、有限なcrossing-window証明書へ
変換した。internal traceをfinal forced upcrossまで延長し、endpointから`return+1`までの値を
`forcedClockSum`で表した。target missingによりcrossingは`a return < target < a (return+1)`とstrictで、
nonempty suffixの`return < target`も保持する。さらにtarget gap `target - a return`とovershoot
...
replay固定点の必要条件を実軌道上で走査する第二の実験（`replay_fixed_point_scan.cpp`）では、
clock 10⁴以下で条件を満たす強制加算クロックは2132個（歩幅2の櫛状鎖として出現）、候補`(clock, target)`対は
約1101万対にのぼり、軌道10¹⁰項の計算でも5,640対（相異なるtarget 18個、すべてRecamánの遅い初出値）が
生き残った。時刻200以内で自分のまたぎ帯を完全被覆できるclockは17ただ一つであり、kernelの`decide`による
有限排除でclockを全域的に引き上げる戦略は成立しない。また着地・blocker条件は実軌道上でひとつの候補対も
排除しなかった。これらは「固定点の排除には非計算的な大域論法が必須」という方針選択の定量的根拠である。
この結果もLean証明には一切取り込んでいない。

第三の実験（`experiments/coverage-limit-2026-08-29/`）は、床上げ機構そのものの漸近的生存可能性を測った。`ReplayPrefixSuccessorCoverage`のcutoffは時刻と値の両方の上界なので最小witnessは初出時刻であり、
`Need(clock) = max { max(v+1, first[v+1]) : v = a t, t < clock, v > a clock }`
とおくと`coverage(clock, cutoff) ⟺ Need(clock) ≤ cutoff`となる。`Need`はcutoff非依存なので、一度計算すれば全cutoffの答えが同時に出る。この再定式化により、記録されていた経験的主張（cutoff 99734でclock 776まで、次の障害はclock 777・successor 879・初出328002）を二つの独立実装で再現・確認した。

kernel射程Xから到達可能なclock床C(X)への換算は次の階段関数になる。

| kernel射程 X | clock床 C(X) | 次の障害clock | 障害successor |
|---:|---:|---:|---:|
| 4,825 | 192 | 193 | 834 |
| 56,515 | 776 | 777 | 879 |
...
回帰すると`X ≈ 0.049 · C^2.01`（R² = 0.87）だが、これは楽観的な平均である。フロンティア付近の局所指数は24,260→53,905で6.29（射程151.6倍でclock 2.2倍）に悪化し、その次は射程を18倍にしても1 clockも進まない。**clock ≈ 5.4×10⁴に機構そのものの天井がある。** clock 53,906はanchor 167,475の上のprefix値167,476（初出t=53,904）の後続167,477が5×10¹⁰まで未出であるため詰まり、射程を伸ばしても抜けない。

天井の理由は構造的である。初出深度比`r(v) = first[v]/v`の裾が極端に重く、値の約0.6%は実用地平線の外に初出を持つ。一方anchorより上のprefix値の個数は`k ≈ 0.17·C`とclockに比例して増えるので、clockが大きいほど「深すぎる後続」を掴む確率が1に近づく。Cmax = 10⁶（eligible clock 213,719個）での消去率は射程10⁵で0.44%、10⁷で2.9%、10⁹で21.1%、5×10¹⁰（全履歴）で34.8%にとどまる。**射程無限大でもclock 10⁶までの65%はcoverageでは消せない。**

固定点の他の必要条件も国勢調査した。Cmax = 10⁶・地平線5×10¹⁰で、条件`C+1 < a C`を満たすclockが713,728個、強制加算も課すと213,739個、そのうち帯`(a C, a (C+1)]`に地平線内未出targetを含むものが212,479個（99.4%）であり、record排除が落とすのはわずか22個である。生き残る`(C, T)`対は4.33×10⁸で、decade別に148 → 5,382 → 1.33×10⁶ → 4.32×10⁸と超線形に増える。帯幅が`C+1`で伸びるため、反例候補の探索空間はclockとともに広がる。

結論として、床上げ路線は「1ラウンドごとに必ず成功するが漸近的に定理へ近づかない」だけでなく、**有限の天井を持つ**。深部軌道値のkernel検証（射程延長）への投資は正当化されない。これらの結果もLean証明には一切取り込んでいない。
...
| prefix successor coverage | `TerminalExactDischargeReplayCertificate.replay_impossible_of_prefixSuccessorCoverage` | `PermanentAboveCorridorPrefixSuccessorCoverage.lean` |
