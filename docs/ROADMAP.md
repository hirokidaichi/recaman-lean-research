...
> 2026-09-07 #73第1pass：[任意周期でlag≤7の供給容量をLean証明](ISSUE73_PERIODIC_SUPPLY_2026-09-07.md)した。
> 全lagの供給不足は未証明。次は長い供給区間を共通の割り当て・potentialへ結び付けられるかを判定する。
> 新しい上限ごとの有限検査・形式化だけでは#73を完了としない。

> 2026-09-07追記：[5方式の並列調査](PARALLEL_APPROACH_TRIAGE_2026-09-07.md)と上位候補の追加反証を実施。
> 次は[issue #73](https://github.com/hirokidaichi/recaman-lean-research/issues/73)：周期符号語の供給不足（E-067/E-070）を証明または反証する一問に絞る。
> 有限lagへの縮約は紙上で監査済みだが、非全射性への接続はまだない。
> 他の方式は今回試した限定classだけを停止し、具体的な独立入力が出た場合に再検討する。
...
## 2026-09-11: 等号側を測り、selector 探索を閉じる

E-070 を広げる代わりに等号が立つ場所を測った。等号語は既証明クラス `E-128` の内側にあり（E-177）、
そこでの課金は一意に強制されて oldest-S と一致する（E-178）。一般には反証された selector が
等号側では唯一解だったので、**新しい selector を探す枝を閉じる**。線形な衝突会計も反証した（E-180）。
次の gate は局所形 T6（高SS窓が内部から S を供出、E-179）を ssCount=2 の最小形で Lean 化すること。
horizon 延長は作業単位にしない。E-067・E-070・全射性／非全射性は未解決。
...
slackを持つ部分集合（$|N(B)| \ge |B| + 1$）は1要素の削除後も $|N(B) \setminus \{s^*(u_0)\}| \ge |B|$ を維持する。
この二分法により、Gate T6供出によるHall条件の保存が全周期的符号語で無条件に成立することが証明された。

さらに、Gate T6帰納容量拡大（E-318）および基底クラス低SS容量 E-128 との統合定理（E-319、`GrandUniversalCapacityResolution`）により：
