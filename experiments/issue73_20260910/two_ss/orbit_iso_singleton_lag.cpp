// Isolated a=0 singleton SS=2: lag histogram, end S, SSS, lag mod 4.
#include <array>
#include <cstdint>
#include <iostream>
#include <stdexcept>
...
int main(){
 ...
 N iso=0, sing=0, endS=0, sss=0;
 std::array<N,4> mod4{};
 I minlag=(1LL<<60), maxlag=0, pend_t=-1, pend_d=-1, pend_endS=0, pend_sss=0;
 std::cout<<"protocol=H-20260910-38 isolated singleton SS=2 lag stats\n"<<std::flush;
 ...
 auto commit=[&](bool nextS){
  ...
  if(nextS){
   ...
   if(pend_sss) ++sss;
   ++mod4[pend_d%4];
   if(pend_d<minlag) minlag=pend_d;
   if(pend_d>maxlag) maxlag=pend_d;
  }
 };
 for(I n=1;n<=limit;++n){
  I t=n-1;bool isS=a>n&&!contains(a-n);
  if(pend_t>=0 && t==pend_t+1){ commit(isS); pend_t=-1; }
  if(!isS){
   auto it=latest.find({P-1,W-t});
   if(it!=latest.end()){
    I d=t-(I)it->second;
    I lead=0; while(lead<d && !signS[t-(lead+1)]) ++lead;
    I ss=0; bool prevS=false, prev2=false, hasSSS=false;
    for(I j=1;j<=d;++j){
     bool s=signS[t-j];
     if(s&&prevS) ++ss;
     if(s&&prevS&&prev2) hasSSS=true;
     prev2=prevS; prevS=s;
    }
    if(ss==2 && lead==0){
     pend_t=t; pend_d=d; pend_endS=signS[t-d]?1:0; pend_sss=hasSSS?1:0;
    }
  }
  }
  if(isS) signS[t]=true;
 ...
 if(pend_t>=0) commit(true);
 if(first19!=99734)throw std::runtime_error("19");
 if(iso!=52198) throw std::runtime_error("iso");
 std::cout<<"ISO={\"iso\":"<<iso<<",\"singleton\":"<<sing
   <<",\"endS\":"<<endS<<",\"hasSSS\":"<<sss
   <<",\"minlag\":"<<minlag<<",\"maxlag\":"<<maxlag
   <<",\"mod4_0\":"<<mod4[0]<<",\"mod4_1\":"<<mod4[1]
   <<",\"mod4_2\":"<<mod4[2]<<",\"mod4_3\":"<<mod4[3]<<"}\n";
}
