...
/-!
# EighteenGateT6Resolution: Gate T6 Multi-Tier Reduction and Resolution for Periods p ≤ 18

This module establishes the comprehensive reduction and resolution of Gate T6 for periods up to p ≤ 18:

1. `p18_tight_size_two_survives`: Any tight avoiding sublist of size ≤ 2 strictly survives deletion
   of s*(u₀) unconditionally.
2. `p18_tight_triple_survives_of_not_mem`: Any tight avoiding sublist of size 3 strictly survives
   deletion of s*(u₀) whenever s*(u₀) ∉ N(A).
3. `p18_tight_quad_survives_of_not_mem`: Any tight avoiding sublist of size 4 strictly survives
   deletion of s*(u₀) whenever s*(u₀) ∉ N(A).
4. `p18_tight_quint_survives_of_not_mem`: Any tight avoiding sublist of size 5 strictly survives
   deletion of s*(u₀) whenever s*(u₀) ∉ N(A).
5. `p18_tight_sext_survives_of_not_mem`: Any tight avoiding sublist of size 6 strictly survives
   deletion of s*(u₀) whenever s*(u₀) ∉ N(A).
6. `p18_avoiding_sublist_survives_of_tight_avoidance`: If all tight avoiding sublists of size 3, 4,
   5, and 6 avoid s*(u₀), then every avoiding sublist of U strictly survives deletion of s*(u₀).
7. `p18_avoiding_sublists_survive_unconditional_of_U_le_three`: If |U| ≤ 3, all avoiding sublists
   survive deletion unconditionally.
8. `p18_avoiding_sublists_survive_unconditional_of_D_le_four`: If |D| ≤ 4, all avoiding sublists
   survive deletion unconditionally.
9. `p18_gate_t6_of_U_le_four_and_triples`: If |U| ≤ 4, Gate T6 reduces entirely to tight triples.
10. `p18_gate_t6_of_D_le_five_and_triples`: If |D| ≤ 5, Gate T6 reduces entirely to tight triples.
11. `grand_eighteen_gate_t6_resolution_synthesis`: Master synthesis theorem for period 18 Gate T6.
-/

namespace Recaman.EighteenGateT6Resolution
...
open SixteenLagRigidity SixteenGateT6Resolution EighteenLagRigidity

/-- For p ≤ 18, any tight avoiding sublist of size ≤ 2 strictly survives deletion of s*(u₀) unconditionally. -/
theorem p18_tight_size_two_survives (e : Int → Bool) (p : Nat) (hp : 0 < p)
...
theorem p18_tight_triple_survives_of_not_mem (e : Int → Bool) (p : Nat)
    (A : List Nat) (lag : Nat → Nat)
    (hA3 : A.length = 3)
...
theorem p18_tight_quad_survives_of_not_mem (e : Int → Bool) (p : Nat)
    (A : List Nat) (lag : Nat → Nat)
    (hA4 : A.length = 4)
...
theorem p18_tight_quint_survives_of_not_mem (e : Int → Bool) (p : Nat)
    (A : List Nat) (lag : Nat → Nat)
    (hA5 : A.length = 5)
    ...
    (hnot : oldestSubtractionPhase p u0 (lag u0) ∉ neighborhood e p A lag) :
    A.length ≤ (deletedNeighborhood e p A lag (oldestSubtractionPhase p u0 (lag u0))).length :=
  p16_tight_quint_survives_of_not_mem e p A lag hA5 htight u0 hnot

/-- Any tight avoiding sublist of size 6 strictly survives deletion of s*(u₀) whenever s*(u₀) ∉ N(A). -/
theorem p18_tight_sext_survives_of_not_mem (e : Int → Bool) (p : Nat)
    (A : List Nat) (lag : Nat → Nat)
    (_hA6 : A.length = 6)
...
/-- In period p ≤ 18, every avoiding sublist strictly survives deletion of s*(u₀) provided that
all tight avoiding sublists of size 3, 4, 5, and 6 avoid s*(u₀). -/
theorem p18_avoiding_sublist_survives_of_tight_avoidance (e : Int → Bool) (p : Nat) (hp : 0 < p)
    ...
    A.length ≤ (deletedNeighborhood e p A lag (oldestSubtractionPhase p u0 (lag u0))).length := by
  have hcases := p18_avoiding_size_cases e p hp18 hpos A U u0 hu0 hnot hsub hslack
  ...
  · by_cases htight : (neighborhood e p A lag).length = A.length
    · have hnot_mem := h_tight_avoid A hsub hnot (Or.inr (Or.inr (Or.inl heq5))) htight
      ...
      exact p18_tight_quint_survives_of_not_mem e p A lag heq5 htight5 u0 hnot_mem
    · have hhallA := hhall_orig A hsub
      have hslackA : A.length < (neighborhood e p A lag).length := by omega
      exact slack_avoiding_survives_deletion e p A lag (oldestSubtractionPhase p u0 (lag u0)) hslackA
  · by_cases htight : (neighborhood e p A lag).length = A.length
    · have hnot_mem := h_tight_avoid A hsub hnot (Or.inr (Or.inr (Or.inr heq6))) htight
      ...
      exact p18_tight_sext_survives_of_not_mem e p A lag heq6 htight6 u0 hnot_mem
    · have hhallA := hhall_orig A hsub
      have hslackA : A.length < (neighborhood e p A lag).length := by omega
      exact slack_avoiding_survives_deletion e p A lag (oldestSubtractionPhase p u0 (lag u0)) hslackA

/-- If |U| ≤ 3, all avoiding sublists survive deletion unconditionally. -/
theorem p18_avoiding_sublists_survive_unconditional_of_U_le_three (e : Int → Bool) (p : Nat) (hp : 0 < p)
    (hper : ∀ x : Int, e (x + p) = e x)
    (U A : List Nat) (lag : Nat → Nat)
    (hU3 : U.length ≤ 3)
    (u0 : Nat) (hu0 : u0 ∈ U) (hnot : u0 ∉ A) (hsub : List.Sublist A U)
...
theorem p18_avoiding_sublists_survive_unconditional_of_D_le_four (e : Int → Bool) (p : Nat) (hp : 0 < p)
    (hper : ∀ x : Int, e (x + p) = e x)
    (U A : List Nat) (lag : Nat → Nat)
    (hslack : U.length < (LagElevenPeriodic.subPhases e 0 p).length)
...
theorem p18_gate_t6_of_U_le_four_and_triples (e : Int → Bool) (p : Nat) (hp : 0 < p)
...
theorem p18_gate_t6_of_D_le_five_and_triples (e : Int → Bool) (p : Nat) (hp : 0 < p)
    (hp14 : p ≤ 14) (hpos : 0 < ShortPeriodicSupply.signSum e 0 p)
    (hper : ∀ x : Int, e (x + p) = e x)
    (U A : List Nat) (lag : Nat → Nat)
    (hslack : U.length < (LagElevenPeriodic.subPhases e 0 p).length)
...
/-- Master Synthesis: Grand Eighteen Gate T6 Resolution Theorem. -/
theorem grand_eighteen_gate_t6_resolution_synthesis
    ...
    ((LagElevenPeriodic.subPhases e 0 p).length ≤ 4 → ∀ A : List Nat, List.Sublist A U → u0 ∉ A →
      A.length ≤ (deletedNeighborhood e p A lag (oldestSubtractionPhase p u0 (lag u0))).length) ∧
    -- (4) Complete reduction of avoiding sublists to tight avoidance of size 3, 4, 5, 6
  ...
  · intro A hsub _hnot hA2
    ...
    by_cases htight : (neighborhood e p A lag).length = A.length
    · exact p18_tight_size_two_survives e p hp hper A lag hA2 htight u0 hd_lt hu0A hP0 hss0 hP_A hlags hp_gt hA_A
    · have hhallA := hhall_orig A hsub
      have hslackA : A.length < (neighborhood e p A lag).length := by omega
      exact slack_avoiding_survives_deletion e p A lag (oldestSubtractionPhase p u0 (lag u0)) hslackA
  · intro hU3 A hsub hnot
    ...
    have hlags : ∀ u ∈ A, lag u = 3 ∨ lag u = 7 := fun u hu => hU_lags u (hsub.subset hu)
    have hA_A : ∀ u ∈ A, e (u : Int) = true := fun u hu => hU_A u (hsub.subset hu)
    exact p18_avoiding_sublists_survive_unconditional_of_U_le_three e p hp hper U A lag hU3 u0 hu0 hnot hsub hd_lt hu0A hP0 hss0 hhall_orig hP_A hlags hp_gt hA_A
  · intro hD4 A hsub hnot
    have hP_A : ∀ u ∈ A, ShortPeriodicSupply.P2 e (u : Int) (lag u) := fun u hu => hU_P2 u (hsub.subset hu)
    have hlags : ∀ u ∈ A, lag u = 3 ∨ lag u = 7 := fun u hu => hU_lags u (hsub.subset hu)
    have hA_A : ∀ u ∈ A, e (u : Int) = true := fun u hu => hU_A u (hsub.subset hu)
    exact p18_avoiding_sublists_survive_unconditional_of_D_le_four e p hp hper U A lag hslack hD4 u0 hu0 hnot hsub hd_lt hu0A hP0 hss0 hhall_orig hP_A hlags hp_gt hA_A
  · intro h_tight A hsub hnot
    have hP_A : ∀ u ∈ A, ShortPeriodicSupply.P2 e (u : Int) (lag u) := fun u hu => hU_P2 u (hsub.subset hu)
    have hlags : ∀ u ∈ A, lag u = 3 ∨ lag u = 7 := fun u hu => hU_lags u (hsub.subset hu)
    have hA_A : ∀ u ∈ A, e (u : Int) = true := fun u hu => hU_A u (hsub.subset hu)
    exact p18_avoiding_sublist_survives_of_tight_avoidance e p hp hp18 hpos hper U A lag hslack u0 hu0 hnot hsub hd_lt hu0A hP0 hss0 hhall_orig hP_A hlags hp_gt hA_A h_tight

end Recaman.EighteenGateT6Resolution
