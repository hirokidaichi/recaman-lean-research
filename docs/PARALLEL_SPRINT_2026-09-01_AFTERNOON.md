...
## 13. 追記: ステップ語の因子センサス（`step_word_census_probe.cpp`、1e9項）

`NoDoubleAdditionRun`を生んだ「histogram空白→定理化」の系統化として、ステップ語（A/S列）の
長さ2〜12の全因子をセンサスした。

- 語は「交互背景＋稀な欠陥」: ASASA型が窓の99.99%、AA/SS欠陥は各~45.8k（0.08%）
- 不在パターン数: L=2..12で 0,0,1,4,14,45,126,320,759,1698,3649
- **最小禁止因子は138個**（L=4: SAAS〔証明済み〕、L=6: AASSAA・SSSSSS…等、L=7: 8個、L=8: 9個…）
- 有力な新定理候補: `AASSAA`（AASSA出現1,476回、全てS継続）、`SAAASAA`、
  `SSSSSAA`/`SSSSSASS`対（S-run≥5の後は位相固定でASAへ再入）
